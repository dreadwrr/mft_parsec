...
void ParseRuns(HANDLE h, unsigned char *run, uint64_t bytesPerCluster, uint16_t bytesPerSector, uint32_t record_size) {
    ...
    while ((unsigned char *)attr < buf + record_size) {
        ...
        if (attr->type == 0x80) {
            if (!attr->non_resident) {
                ...
                ParseRuns(h, run, bytesPerCluster, bootsector.bytesPerSector, record_size);
